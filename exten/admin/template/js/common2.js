...
var setting = {
	...
	uploadFileExtension: "xls|xlsx|csv"
};
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: shopxx.base + "admin.php?app=materialTable&act=excelFileList",
				uploadUrl: shopxx.base + "admin.php?app=materialTable&act=uploadExcel",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					$browserOrderType = $('<select name="orderType" class="browserOrderType"><option value="name">' + message("admin.browser.name") + '<\/option><option value="time">' + message("admin.browser.time") + '<\/option><option value="size">' + message("admin.browser.size") + '<\/option><\/select>').appendTo($browserBar);
					...
					function createBrowserList(path, data) {
						var browserListHtml = "<ul>";
						$.each(data, function(i, fileInfo) {
							var iconUrl;
							var title;
							if (fileInfo.isDirectory) {
								iconUrl = shopxx.base + "exten/admin/template/images/folder_icon.gif";
								title = fileInfo.name;
							} else if (new RegExp("^\\S.*\\.(jpg|jpeg|bmp|gif|png)$", "i").test(fileInfo.name)) {
								iconUrl = fileInfo.url;
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + fileInfo.lastModified + ")";
							} else {
								iconUrl = shopxx.base + "exten/admin/template/images/folder_icon.gif";
								title = Math.ceil(fileInfo.size / 1024) + "KB, " + fileInfo.lastModified;
							}
							browserListHtml += '<li url="' + fileInfo.url + '" style="height:30px; padding-left:10px; list-style:none; border-bottom:1px dashed #b7d3e1; cursor:pointer"><div style="line-height:30px; float:left; width:220px; color:#333333">'+fileInfo.name+'</div><div style="line-height:30px; float:left; width:130px">'+fileInfo.lastModified+'</div><div style="line-height:30px; float:left;">'+Math.ceil(fileInfo.size / 1024)+"KB"+'</div></li></ul>';
						});
						browserListHtml += "</ul>";
						$browserList.html(browserListHtml);
						
						$browserList.find("li").bind("click", function() {
							var $this = $(this);
