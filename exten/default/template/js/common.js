	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: shopxx.base + "?app=system&act=browse",
				uploadUrl: shopxx.base + "?app=system&act=upload",
				callback: null
			};
			$.extend(settings, options);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = shopxx.base + "/material/exten/admin/template/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = shopxx.base + "/material/exten/admin/template/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
